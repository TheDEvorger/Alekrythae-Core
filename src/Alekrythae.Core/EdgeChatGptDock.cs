...
{
    /// <summary>
    /// Gerçek Google Chrome'un uygulama modu penceresini Meggy'nin AI çalışma
    /// alanına görsel olarak kenetler. Chrome ayrı bir tarayıcı işlemi ve kendi
    ...
    public sealed class EdgeChatGptDock : IDisposable
    {
        ...
        public async Task OpenAsync()
        {
            ...
            // Yeni veya daha önce gizlenmiş Chrome penceresinin eski ekran
        ...
        {
            string edgePath = FindGoogleChrome();
            HashSet<IntPtr> existingWindows = SnapshotChromeWindows();
            string cursorExtensionPath = Path.Combine(AppContext.BaseDirectory, "Resources", "AlekrythaeCursorExtension");
            string cursorExtensionArgument = Directory.Exists(cursorExtensionPath)
                ? "--load-extension=\"" + cursorExtensionPath + "\" "
                : "";
            ...
            for (int attempt = 0; attempt < 100; attempt++)
            {
                ...
                foreach (IntPtr window in SnapshotChromeWindows())
                {
                ...
                // görünür olan Chrome penceresini birkaç saniye kararlı kaldıktan
            ...
            throw new InvalidOperationException(
                "Google Chrome açıldı fakat ChatGPT uygulama penceresi bulunamadı. " +
                "Açık Chrome uygulama pencerelerini kapatıp yeniden deneyin.");
        }
        ...
        private void ApplyDockStyle()
        {
            ...
            try
            {
                // Chrome gezinme veya profil başlangıcı sırasında pencere stilini
                ...
                // SetParent kullanılmaz. Chrome gerçek bir üst seviye tarayıcı
                ...
                {
                    throw new InvalidOperationException(
                        "Google Chrome penceresi Meggy'ye güvenli biçimde kenetlenemedi.");
                }
        ...
        private void SyncWindowBounds()
        {
            ...
            try
            {
                ...
                // Chrome --app penceresi klasik WS_CAPTION kaldırıldıktan sonra
        ...
        private bool IsLiveEdgeWindow()
        {
            return _edgeWindow != IntPtr.Zero &&
                   ...
                   IsGoogleChromeWindow(_edgeWindow);
        }

        private static HashSet<IntPtr> SnapshotChromeWindows()
        {
            var result = new HashSet<IntPtr>();
            EnumWindows((window, _) =>
            {
                if (IsGoogleChromeWindow(window))
                    result.Add(window);
                return true;
            }, IntPtr.Zero);
            return result;
        ...
        private static bool IsGoogleChromeWindow(IntPtr window)
        {
        ...
        private static string FindGoogleChrome()
        {
            var candidates = new List<string>();
            AddChromeCandidate(candidates, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86));
            AddChromeCandidate(candidates, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles));
            AddChromeCandidate(candidates, Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData));
            ...
            {
                object? appPath = Registry.GetValue(hive, "", null);
            ...
            foreach (string candidate in candidates.Distinct(StringComparer.OrdinalIgnoreCase))
            {
                try
                {
                    if (!File.Exists(candidate) ||
                        !string.Equals(Path.GetFileName(candidate), "chrome.exe", StringComparison.OrdinalIgnoreCase))
                        continue;
                    return candidate;
                }
                catch
                {
                    // Sonraki bilinen Chrome kurulum konumunu dene.
                }
            }

            throw new FileNotFoundException(
                "Google Chrome bulunamadı. Chrome'u kurup yeniden deneyin.");
        }

        private static void AddChromeCandidate(List<string> candidates, string root)
        {
