...
namespace AlekrythaeCore
{
    ...
    internal sealed class DeveloperBridge : IDisposable
    {
                ...
                switch (op)
                {
                    case "dev.status":
                        result = new
                        {
                            ...
                            compatibilityMode = "additive-dev-namespace",
                            legacyApisUntouched = true,
                            gpuSelectorCompat = true
                        };
                        return true;
        ...
        private object StartShell(JsonElement payload)
        {
            ...
            int requestedColumns = GetInt(payload, "columns", 140);
            int requestedRows = GetInt(payload, "rows", 36);

            short columns = (short)Math.Clamp(requestedColumns, 20, 400);
            short rows = (short)Math.Clamp(requestedRows, 5, 160);

            string shellId = Guid.NewGuid().ToString("N");
            ConPtySession session = ConPtySession.Start(
                workingDirectory,
                columns,
                rows);
            ...
            // Do not announce a terminal that already died during ConPTY startup.
            // The exit event is intentionally NOT wired yet, so a failed startup
            // cannot leak a misleading asynchronous "[process exited]" event.
            if (session.WaitForEarlyExit(120, out int earlyExitCode))
            {
                ...
                {
                    ok = false,
                    error = "conpty_child_exited_during_startup",
                    exitCode = earlyExitCode,
                    exitCodeHex = unchecked((uint)earlyExitCode).ToString("X8")
                };
            }
            ...
            // Cover the tiny race between the startup probe and event wiring.
            if (session.TryGetExitCode(out int racedExitCode))
            {
                ...
                return new
                {
                    ...
                    exitCodeHex = unchecked((uint)racedExitCode).ToString("X8")
                };
            }
