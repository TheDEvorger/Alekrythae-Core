...
namespace AlekrythaeCore
{
    ...
    internal sealed class ConPtySession : IDisposable
    {
        ...
        private readonly BlockingCollection<string> _writeQueue = new();
        ...
        private void StartCore(string workingDirectory, short columns, short rows)
        {
            ...
            try
            {
                ...
                // Keep writes off the WebView/UI thread as well.
                _writeCts = new CancellationTokenSource();
                _writeTask = Task.Factory.StartNew(
                    () => WriteLoop(_writeCts.Token),
                    CancellationToken.None,
                    TaskCreationOptions.LongRunning,
                    TaskScheduler.Default);
            }
        ...
        public void Write(string text)
        {
            ...
            if (_writer == null || _writeCts == null)
                throw new InvalidOperationException("conpty_writer_not_ready");

            try
            {
                _writeQueue.Add(text ?? string.Empty, _writeCts.Token);
            }
            catch (InvalidOperationException)
            {
                throw new InvalidOperationException("conpty_writer_closed");
            }
        }
        ...
        public bool WaitForEarlyExit(int milliseconds, out int exitCode)
        {
            exitCode = -1;
            ...
            try
            {
                if (!process.WaitForExit(Math.Max(0, milliseconds)))
                    return false;
            ...
            {
                return TryGetExitCode(out exitCode);
            }
        }
        ...
        private void WriteLoop(CancellationToken token)
        {
            try
            {
                foreach (string text in _writeQueue.GetConsumingEnumerable(token))
                {
                    if (_writer == null)
                        break;
        ...
        public void Dispose()
        {
            ...
            // Closing input first tells the hosted shell there will be no more commands.
            ...
            // Close the pseudoconsole before tearing down the output channel.
            // Its final frame/broken-pipe transition can still be drained by ReadLoop.
